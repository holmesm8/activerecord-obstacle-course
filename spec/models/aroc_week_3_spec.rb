...
# ----------------------
  ...
  it '17. returns the name of items associated with a specific order' do
    ...
    # Solution goes here
    order = Order.last
    names = Item.joins(:orders).where("order_id = #{order.id}").pluck(:name)
    # ------------------------------------------------------------
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    order = User.find(@user_3.id).orders.limit(3).last
    items_for_user_3_third_order = Item.joins(:order_items).where("order_id = #{order.id}").pluck(:name)
    # ------------------------------------------------------------
  ...
  it '20. returns the average amount for all orders for one user' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    average = Order.joins(:order_items).where("user_id = #{@user_3.id}").average(:amount)
    # ------------------------------------------------------------
